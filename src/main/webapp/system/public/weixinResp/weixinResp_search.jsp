...
<html>
	<head>
		...
		function del(id){
			if(confirm("确定删除吗?")){
				window.self.location='weixinResp_delete.action?id='+id;
			}
		}
		function init() {
			hiddenBtn('system/weixinResp_edit.action','edit');
			hiddenBtn('system/weixinResp_delete.action','del');
			hiddenBtn('system/weixinResp_add.action','add');
		}
		</script>		
	...
	<body onload="init();">
		<div class="headerPart">
  			...
  			<span><font class="font1">当前位置:</font><font class="menuName_1">微信菜单回复</font></span>
        </div>
    	<div class="main">
			<s:form action="weixinResp_search" theme="simple" method="post"	namespace="/system">
				<table class="tableborder" border="1" bordercolor="#dbdbdb">
				    <tr>
					    <td class="th1" style="display: none">选择商户:</td>
						<td align="left" colspan="3" style="display: none"><span id="customerList"></td>
				    </tr>
					<tr>
						<td class="th1">
							回复标题:
						...
						<td width="35%" align="left">
							<s:select id="type" name="type" list="#{'': '全部','text': '文本消息','news': '图文消息'}"></s:select>
						</td>
					</tr>
					<tr>
						<td class="th1">
						...
						<td width="35%" align="left">
							<s:textfield id="picName" name="picName"></s:textfield>
						</td>
					</tr>
				</table>
				...
				<table width="100%">
					<tr>
						...
						<td align="left" width="50%">
							<input type="button" class="add_btn" onmousemove="this.className='add_btn1'" onmouseout="this.className='add_btn'" value="" onclick="javascript:window.location.href='<%=request.getContextPath() %>/system/public/weixinResp/weixinResp_add.jsp'"/>
						</td>
					</tr>
				</table>
						...
						<s:else>
							<s:iterator value="pageList" status="st" id="da">
								<tr class="<s:if test="#st.even">row-even</s:if><s:else>row-odd</s:else>">
									...
									<td align="center">
										<s:if test="type=='text'">文本消息</s:if>
										<s:elseif test="type=='news'">图文消息</s:elseif>
									</td>
									<td align="center">
										<s:property value="description" />
									</td>
									<td align="center">
										<s:property value="keyId" />									
									</td>
							...
							<s:if test="pageList != null && pageList.size>0">
								<tr class="pageTr">
									<td colspan="8">
										<s:url id="url_pre" value="weixinResp_search.action">
										...
										<s:url id="url_next" value="weixinResp_search.action">
											<s:param name="pageIndex" value="pageIndex+1"></s:param>
											<s:param name="title" value="title"></s:param>
											<s:param name="type" value="type"></s:param>
											<s:param name="keyId" value="keyId"></s:param>
										...
										<s:url id="url_first" value="weixinResp_search.action">
										...
										<s:url id="url_last" value="weixinResp_search.action">
