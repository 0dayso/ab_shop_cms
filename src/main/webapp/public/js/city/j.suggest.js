			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					for (var i = 0; i < options.source.length; i++) {//国内城市匹配
						...
						if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2]) || reg.test(options.source[i][3]) || reg.test(options.source[i][1]+options.source[i][2])) {
							html += '<li rel="' + options.source[i][0] + '"><a href="#' + i + '"><span>' + options.source[i][1] + '</span>' + options.source[i][2] + '</a></li>';
						}
					}
					if (html == '') {
...
//				}
			    ...
			    //alert($currentResult.children('a').html());
			    var abcd = $currentResult.children('a').html();
			    var abcd1 = abcd.substring(6,abcd.length);
			    //<span>PEK</span>北京
			    //PEK</span>北京
			    //alert(abcd1);
			    //alert("ok1");
			    //PEK北京
			    var abcd2 = abcd1.substring(0,3);
			    var abcd3 = abcd1.substring(10,abcd1.length);
			    //alert(abcd2);
			    //alert(abcd3);
			    var abcd4 = abcd2.concat(abcd3);
				if ($currentResult) {
					//alert(abcd.replace(/<span><\/span>/gi,""));
					//$input.val(abcd.replace(/<span>.+?<\/span>/,""));
					//alert(abcd);
					$input.val(abcd4);
					//$input.val($currentResult.children('a').html().replace("<span></span>","abc"));
					$results.hide();
