        ...
        popup: function(options) {
            ...
            return this.each(function() {
				...
				var item = $('<div id="dhauthordiv"></div>').width(width).height(height)
					...
					$('.tite').find('dd:first').attr('b_YN_icon','y').addClass('ynicon').width(30).css({float:'left',background:'url(' + path + '/images/tag.png) 3px 2px no-repeat'});
					$('.tite').find('dd:last').width(20).css({float:'right',background:'url(' + path + '/images/tag.png) -30px 2px no-repeat',cursor:'pointer'});
		...
		tip: function(options){
			...
			var options = $.extend({},defaults, options);
			 return this.each(function() {
				...
				var item = $('<div id="tip-box"><em id="closess-status" class="closess-status" title="关闭"></em><span>'+d_content+'</span><em id="tip-status" class="tip-status"></em></div>').width(d_width).height(d_height).css({display:'none',fontFamily:'&#23435;&#20307;',fontSize:'12'+'px',color:'#fff',borderRadius:'5px',lineHeight:d_height+'px'});
					 $('body').append(item);
