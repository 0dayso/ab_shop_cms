...
public class FlightUtil {
	...
	public static String genSelectedForSearch(Object defaultSelect,
			String parameterName, boolean isReadOnly, Object[] codes,
			String[] view) {
		StringBuffer sb = new StringBuffer();
		String readOnly = isReadOnly ? " disabled='disabled' " : "";
		sb.append("<select name=\"" + parameterName + "\" " + readOnly
				+ "id=\"" + parameterName + "\">");
		sb.append("<option value='-1' >全部</option>");
		for (int i = 0; i < codes.length; i++) {
			if (codes[i].equals(defaultSelect) || view[i].equals(defaultSelect)) {
				sb.append("<option value=\"" + codes[i]
						+ "\" selected=\"true\">" + view[i] + "</option>");
			} else {
				sb.append("<option value=\"" + codes[i] + "\">" + view[i]
						+ "</option>");
			}
		}
		sb.append("</select>");
	...
	public static String genSelectedForChange(Object defaultSelect,
			String parameterName, boolean isReadOnly, Object[] codes,
			String[] view) {
		StringBuffer sb = new StringBuffer();
		String readOnly = isReadOnly ? " disabled='disabled' " : "";
		...
		for (int i = 0; i < codes.length; i++) {
			if (codes[i].equals(defaultSelect) || view[i].equals(defaultSelect)) {
				sb.append("<option value=\"" + codes[i]
						+ "\" selected=\"selected\">" + view[i] + "</option>");
			} else {
				sb.append("<option value=\"" + codes[i] + "\">" + view[i]
						+ "</option>");
			}
	...
	public static String getCode(){
		...
		Integer code = 100000 + random.nextInt(99999);
